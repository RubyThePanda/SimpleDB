...
public class Aggregate extends AbstractDbIterator {
    ...
    public Aggregate(DbIterator child, int afield, int gfield, Aggregator.Op aop) {
        ...
        Type aggType = childTD.getType(afield);
        Type groupType;
     ...
     */
        ...
        if (gfield == Aggregator.NO_GROUPING) {
            ...
            fieldNames = new String[]{
                    aggName(aop) + "(" + child.getTupleDesc().getFieldName(afield) + ")"
            };
        } else {
            groupType = childTD.getType(gfield);
        ...
        resTD = new TupleDesc(types, fieldNames);
        if (aggType == Type.INT_TYPE) {
            aggregate = new IntAggregator(gfield, groupType, afield, aop);
    ...
    public void open()
        ...
        child.open();
        while(child.hasNext()) {
            aggregate.merge(child.next());
        }
        child.close();
        aggIter = aggregate.iterator();
        aggIter.open();
    }
