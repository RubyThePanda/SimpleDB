...
public class HeapPage implements Page {
    ...
    */
    private int getNumTuples() {
        // floor((BufferPool.PAGE_SIZE*8) / (tuple size * 8 + 1))
        return (int)Math.floor((BufferPool.PAGE_SIZE * 8) / (double)(td.getSize() * 8 + 1));
    }
    ...
    private int getZeroesInByte(byte input) {
        byte counter = 0;
    ...
    public int getNumEmptySlots() {
        int enddingHeaderZeroes = header.length * 8 - numSlots;
        int num = 0;
        for (byte head : header) {
            num += getZeroesInByte(head);
        }
        return num - enddingHeaderZeroes;
    }
    ...
    public boolean getSlot(int i) {
        ...
        int byteIndex = i / 8;
        int posInByte = i % 8;
        return ((header[byteIndex] >> posInByte) & 1) == 1 ? true : false;
    }
    ...
    private class TupleIterator implements Iterator<Tuple> {
        ...
        public boolean hasNext() {
            while(!getSlot(cursor) && cursor < numSlots) {
                ++cursor;
            }
